...
with open('score_list_svm.pkl', 'rb') as score_list:
    unpickle_score_list_svm_classification = pickle.load(score_list)

with open('score_list_regression.pkl', 'rb') as score_list:
    unpickle_score_list_svm_regression = pickle.load(score_list)


accuracy_means = np.array([1 - np.mean(i['test_accuracy']) for i in unpickle_score_list_svm_classification]) #Calculate the means out of the returned accuracy from the standard svm
mse_means = np.array([np.mean(i['test_neg_mean_squared_error']) for i in unpickle_score_list_svm_regression])
print(mse_means)
